...
// ==========================================
// A MÁGICA: ESCUTAR A JANELA DA CALCULADORA (IFRAME)
// ==========================================
window.addEventListener('message', (event) => {
    // Se o iframe mandou a mensagem de cálculo finalizado, nós injetamos na tela
    if (event.data && event.data.tipo === 'CALCULO_FOLHA_FINALIZADO') {
        const liquido = event.data.liquido;
        const adiantamento = event.data.adiantamento;
        ...
        // Fecha a sanfona
        document.getElementById('iframe-calculadora').style.display = 'none';
...
window.abrirModulo = (modulo) => {
    ...
    document.getElementById('app-title').innerText = titulos[modulo] || 'JB Finance Analytics V4.0';
