...
// ==========================================
// 0. MOTOR DA CALCULADORA MODULAR (BLINDADO)
// ==========================================
const regrasCalculo = {
    anoVigencia: 2026,
    tetoINSS: 8475.55,
...
function calcularINSS(base) {
    if (base > regrasCalculo.tetoINSS) {
        base = regrasCalculo.tetoINSS;
    }
    for (const faixa of regrasCalculo.tabelaINSS) {
        if (base <= faixa.ate) {
            return (base * faixa.aliquota) - faixa.deduzir;
        }
    }
...
window.calcularEInjetarModular = () => {
    ...
    if(salarioBase <= 0) {
        alert("Configure o Salário Base Contratual primeiro logo acima!");
        return;
    }

    // Captura as variáveis da sanfona
    ...
    // Total bruto (Simplificação do DSR para esta injeção)
    ...
    // Fecha a sanfona visualmente
    const headerSanfona = document.querySelector('.accordion-header.active');
    if(headerSanfona) {
        headerSanfona.classList.remove('active');
        headerSanfona.nextElementSibling.classList.remove('open');
    }
};
...
window.carregarTodosOsDados = async () => {
    try {
        ...
        try {
            ...
            console.log("Aviso config modular:", e); 
        }
...
window.salvarSalarioBase = async () => {
    ...
    try {
        ...
        window.mostrarToast("Salário Base salvo com sucesso!");
    } catch(e) { 
        console.error(e); 
        alert("Erro ao salvar Salário Base"); 
    }
};
...
window.salvarMesModular = async () => {
    ...
    if (!mesStr) {
        return alert("Selecione o mês de referência.");
    }

    const salarioBase = parseFloat(localStorage.getItem('modular_salario_base')) || 0;
    if (salarioBase <= 0) {
        return alert("Configure e salve o seu Salário Base primeiro!");
    }
...
window.renderizarHistoricoModular = () => {
    ...
    if (regs.length === 0) { 
        container.innerHTML = "<p style='text-align:center;'>Sem dados encontrados.</p>"; 
        return; 
    }
    ...
    regs.forEach(r => { 
        ...
        tableHtml += `
            <tr style="border-bottom: 1px solid #eceff1;">
                <td style="padding: 10px 5px; text-align: left; font-weight: bold; color: #455a64;">
                    ${MESES[r.mes]} ${r.ano}
                </td>
                ...
                <td style="padding: 10px 5px;">
                    <span style="color:red; cursor:pointer; font-size:14px;" onclick="window.excluirRegistroModular('${r.id}')">✖</span>
                </td>
            </tr>`; 
    });
...
window.renderizarDashboardGeral = () => {
    ...
    window.registros.forEach(r => { 
        ...
        dadosGerais[k].saripan += parseFloat(r.total) || 0; 
    });
    ...
    window.registrosExtra.forEach(r => { 
        const k = initData(r.ano, r.mes); 
        dadosGerais[k].extra += parseFloat(r.total) || 0; 
    });

    const anosOrdenados = Array.from(anosEncontrados).sort((a,b) => b-a);
    
    if(anosOrdenados.length === 0) { 
        container.innerHTML = "<p style='text-align:center;'>Sem dados encontrados para o dashboard.</p>"; 
        return; 
    }
        ...
        mesesDoAno.forEach(m => {
            ...
            htmlTabelaCorpo += `
                <tr>
                    ...
                    <td class="esconder-valor" style="padding: 8px; text-align: right; background: #e8f5e9; font-weight: bold; color: #1b5e20;">
        ...
        <div style="margin-bottom: 35px;">
            ...
            <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                <div class="year-summary" style="flex: 1; padding: 10px; border-color: #ffcc80;">
                    ...
                    <div class="year-total-value esconder-valor" style="font-size: 15px; margin-top: 10px; color: #e65100;">
                        ${totalAcumuladoAno.toLocaleString('pt-BR', {style:'currency', currency:'BRL'})}
                    </div>
                </div>
                
                <div class="year-summary" style="flex: 1; padding: 10px; background: #e3f2fd; border-color: #90caf9;">
                    <h4>MÉDIA MENSAL</h4>
                    <div class="year-total-value esconder-valor" style="font-size: 15px; color: #0d47a1; margin-top: 10px;">
    ...
    let htmlRows = regs.map(r => {
        ...
        return `
            <tr>
                ...
                <td class="td-valor esconder-valor" style="color:#f57c00; font-weight:bold; padding: 10px;">R$ ${r.total.toFixed(2)}</td>
                <td style="padding: 10px; text-align:center;">
                    <span style="color:red; cursor:pointer;" onclick="window.excluirRegistroExtra('${r.id}')">✖</span>
                </td>
            </tr>`;
    }).join('');
...
window.atualizarPreview = () => {
    ...
    if (previewEl) {
        previewEl.value = `R$ ${(base * multiplicador).toFixed(2)}`;
    }
};
...
window.excluirQuinzena = async (chaveGrupo) => {
    ...
    try {
        ...
        console.error("Erro ao excluir quinzena", e); 
...
window.renderizarFinanceiroSaripan = () => {
    ...
    if (chavesOrdenadasMes.length === 0) { 
        container.innerHTML = "<p style='text-align:center; padding: 20px; color:#999;'>Sem dados financeiros.</p>"; 
        return; 
    }
    ...
    anosOrdenados.forEach(anoStr => {
        ...
        htmlFinal += `
            <h4 style="margin-top: 10px; color: #555; border-bottom: 2px solid #ddd; padding-bottom: 5px; text-transform: uppercase;">
                Resumo Saripan ${ano}
            </h4>
            <div style="display: flex; gap: 10px; margin-bottom: 15px;">
                <div class="year-summary" style="flex: 1; padding: 10px;">
                    <h4>RENDIMENTO ANUAL</h4>
                    <div class="year-total-value esconder-valor" style="font-size: 17px; margin-top: 10px;">
                        ${totalAcumuladoDoAno.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}
                    </div>
                </div>
                <div class="year-summary" style="flex: 1.8; padding: 10px; background: #e3f2fd; border-color: #90caf9;">
                    ...
                    <div style="display: flex; justify-content: space-around; font-size: 14px; color: #0d47a1;">
                        <div style="text-align: center;">
                            ...
                            <strong class="esconder-valor" style="font-size: 15px;">
                                ${mediaParcialFechada.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}
                            </strong>
                        </div>
                        ...
                        <div style="text-align: center;">
                            ...
                            <strong class="esconder-valor" style="font-size: 15px;">
                                ${mediaTotalProporcional.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'})}
                            </strong>
                        </div>
                    </div>
                </div>
            ...
            <div style="background: white; padding: 15px; border-radius: 8px; border: 1px solid #ddd; margin-bottom: 15px;">
                <h5 style="margin-bottom: 10px; color: #333; font-size: 12px; text-align: center;">
                    Rendimento Mensal Consolidado
                </h5>
                ...
                <h5 style="margin-bottom: 10px; color: #333; font-size: 12px; text-align: center; border-top: 1px dashed #eee; padding-top: 15px;">
                    Comparativo Quinzenal (Q1 vs Q2)
                </h5>
