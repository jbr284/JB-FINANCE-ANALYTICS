...
window.atualizarRodapeDinamico = () => {
    ...
    if(rodapeQtd) rodapeQtd.innerText = qtd;
    if(rodapeTotal) rodapeTotal.innerText = tot.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'});
...
window.adicionarRegistro = async () => {
    ...
    try {
        ...
        window.renderizarFinanceiroSaripan();
        window.renderizarDashboardGeral();
        window.mostrarToast("Diária registrada!"); 
    } catch(e) { console.error(e); }
};
...
window.adicionarRegistroModular = async () => {
    ...
    const totalBeneficiosFixos = 600 + 500 + 125;
    ...
    try {
        ...
        window.renderizarHistoricoModular();
        window.renderizarDashboardGeral();
        window.mostrarToast("Modular (Salário e Benefícios) Salvos!");
    } catch(e) { console.error(e); }
};
...
window.renderizarDashboardGeral = () => {
    ...
    window.registrosModular.forEach(r => { 
        ...
        dadosGerais[k].beneficios += (r.totalBeneficios || 0); 
    });
    ...
    anosOrdenados.forEach(ano => {
        ...
        mesesDoAno.forEach(m => {
            ...
            const totalRemuneracaoMes = m.saripan + m.modular + m.extra; 
            ...
            if(ctx) {
                const chart = new Chart(ctx, { 
                    ...
                    data: { labels: labels, datasets: [ 
                        ...
                        { label: 'Benefícios', data: dataBen, backgroundColor: '#81c784' } 
                    ] }, 
